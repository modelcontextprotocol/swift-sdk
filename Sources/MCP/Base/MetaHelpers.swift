...
enum MetaFieldError: Error, LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .invalidMetaKey(let key):
            return "Invalid _meta key: '\(key)'. Keys must follow the format: [prefix/]name where prefix is dot-separated labels and name is alphanumeric with hyphens, underscores, or dots."
        }
    }
}
...
func encodeMeta(_ meta: [String: Value]?, to container: inout KeyedEncodingContainer<DynamicCodingKey>) throws {
    ...
    var metaContainer = container.nestedContainer(keyedBy: DynamicCodingKey.self, forKey: metaCodingKey)
...
func encodeExtraFields(_ extraFields: [String: Value]?, to container: inout KeyedEncodingContainer<DynamicCodingKey>, excluding excludedKeys: Set<String> = []) throws {
...
func decodeMeta(from container: KeyedDecodingContainer<DynamicCodingKey>) throws -> [String: Value]? {
    ...
    let metaContainer = try container.nestedContainer(keyedBy: DynamicCodingKey.self, forKey: metaCodingKey)
...
func decodeExtraFields(from container: KeyedDecodingContainer<DynamicCodingKey>, excluding excludedKeys: Set<String> = []) throws -> [String: Value]? {
    var extraFields: [String: Value] = [:]
    
    for key in container.allKeys where key.stringValue != metaKey && !excludedKeys.contains(key.stringValue) {
