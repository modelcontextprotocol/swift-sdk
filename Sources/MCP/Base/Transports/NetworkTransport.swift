...
import Logging
    ...
    /// A thread-safe, `Sendable` flag that guards against resuming a
    /// continuation more than once.
    ///
    /// This replaces bare `var resumed = false` locals that were captured
    /// across isolation domains (e.g. actor → `@MainActor`), which Swift 6.3
    /// correctly flags as a data-race risk (`#SendingRisksDataRace`).
            ...
            try await withCheckedThrowingContinuation {
                ...
                connection.send(
                    ...
                    completion: .contentProcessed { [weak self] error in
                        ...
                        Task {
                            guard sendGuard.tryMarkResumed() else { return }
                            if let error = error {
                                await self.logger.error("Send error: \(error)")
        ...
        private func receiveData() async throws -> Data {
            ...
            return try await withCheckedThrowingContinuation {
                ...
                connection.receive(minimumIncompleteLength: 1, maximumLength: maxLength) {
                    ...
                    Task {
                        guard receiveGuard.tryMarkResumed() else { return }
                        if let error = error {
                            continuation.resume(throwing: MCPError.transportError(error))
                        } else if let content = content {
                            continuation.resume(returning: content)
                        } else if isComplete {
                            await self.logger.trace("Connection completed by peer")
